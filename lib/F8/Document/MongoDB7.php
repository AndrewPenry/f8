...
trait MongoDB7
{
     ...
     *  collation       array. Allows for custom sorting. See
     *  https://docs.mongodb.com/manual/reference/collation/#collation-document comment         string. For profiling.
     *  maxTimeMS       int. Time limit for processing cursor Deprecated options fields          Mongo field
     *  projection, but only positive. Use projection instead
     *
     * @param array                $options
    ...
    public function search(array $options = [], &$errors = [], &$count = null, $db = null)
    {
        ...
        try {
            ...
            echo $e->getMessage();
            $locator->logger('f8')->error('Mongo Exception',
                ['exception' => $e->getMessage(), 'code' => $e->getCode()]);
            /** @var ErrorFactory $mf */
            $mf = $locator->locate('messageFactory', 'app');
            $errors[] = $mf->message(_("Error while searching"), 803001, ['document-type' => get_class($this)]);
        }
    ...
    public function create(array $options = [], &$errors = [], $db = null)
    {
        ...
        try {
            ...
            /** @var ErrorFactory $mf */
            $mf = $locator->locate('messageFactory', 'app');
            $errors[] = $mf->message(_("Document could not be created"), 803002, ['document-type' => get_class($this)]);
        }
        ...
        try {
            ...
            if (empty($options)) {
                if ($db->read($this->getMongoCollection(), $this) === false) {
                    $errors[] = $mf->message(_("Document Not Found"), 803001, ['document-type' => get_class($this)]);
                }
                return $this;
            }
        ...
        try {
            ...
            if (empty($options)) {
                if ($db->delete($this->getMongoCollection(), $this) === false) {
                    $errors[] = $mf->message(_("Document Not Deleted"), 803008, ['document-type' => get_class($this)]);
                    return false;
                }
                return true;
            }
    ...
    public function save(array $options = [], &$errors = [], $db = null)
    {
        ...
        try {
            ...
            if ($result->getModifiedCount()) {
                ...
                $errors[] = $mf->message(_("Document Not Modified"), 803006, ['document-type' => get_class($this)]);
                return $this;
            }
        } catch (Exception $e) {
            $locator->logger('f8')->error('Mongo Exception',
                ['exception' => $e->getMessage(), 'code' => $e->getCode()]);
            $errors[] = $mf->message(_("Document Not Modified"), 803006,
                ['document-type' => get_class($this), 'mongoException' => $e]);
            return $this;
        } catch (\Exception $e) {
            $locator->logger('f8')->error('Mongo Exception',
                ['exception' => $e->getMessage(), 'code' => $e->getCode()]);
